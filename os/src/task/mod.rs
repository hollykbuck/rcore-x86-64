...
/// Block the current 'Running' thread, returning the pointer to its saved
/// task context. **Must be followed by a call to [`schedule`]** with that
/// pointer (ch9: used when the caller holds a lock that must be released
