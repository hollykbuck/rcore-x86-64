...
pub struct PerCpu {
    ...
    /// interrupt-masking nesting depth for `UPIntrFreeCell` (irqsave). Only
    /// touched by `sync::up`. Per-core (not global!) because on SMP each core
    /// must independently track whether IT enabled interrupts before taking a
    /// lock (RISC-V ch9 keeps this in a single global, which only works on a
    /// uniprocessor).
    pub irq_nested: usize,
    /// whether `RFLAGS.IF` was set before the outermost `irqsave_enter`, so
    /// `irqsave_exit` can restore it once the nesting depth reaches zero.
    pub irq_saved_if: bool,
}
