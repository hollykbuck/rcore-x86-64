...
const EFER_NXE: u64 = 1 << 11; // enables the NX (not-executable) page bit
...
/// Enable `EFER.NXE`. Without it the NX page-table bit is silently ignored:
/// read-only data (and user stacks) would be executable.
pub fn enable_nxe() {
    unsafe {
        let efer = rdmsr(EFER_MSR);
