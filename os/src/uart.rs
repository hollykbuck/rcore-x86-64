...
/// Try to read a byte from the serial console. Returns `0` if no data is
/// available (the analogue of the RISC-V `console_getchar`).
pub fn console_getchar() -> u8 {
    unsafe {
        let status: u8;
        asm!("in al, dx", out("al") status, in("dx") 0x3F8 + 5);
        if status & 0x01 != 0 {
            // line status register: receiver data ready
            let c: u8;
            asm!("in al, dx", out("al") c, in("dx") 0x3F8 + 0);
            c
        } else {
            0
        }
